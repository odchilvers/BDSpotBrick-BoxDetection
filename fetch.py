...
# bosdyn.client.manipulation_api_client import ManipulationApiClient
...
def get_obj_and_img(network_compute_client, server, model, confidence,
    ...
    for source in image_sources:
        ...
        if best_obj is not None:
            return best_obj, image_full, best_vision_tform_obj, resp.object_in_image
...
def main(argv):
    ...
    parser.add_argument('-m',
                        ...
                        required=True)
    parser.add_argument( # might need to name this box to first find our box!
        '-p',
        '--person-model',
        help='Person detection model name running on the external server.')
    parser.add_argument('-c',
                        '--confidence-brick',
                        help='Minimum confidence to return an object for the brick (0.0 to 1.0)',
                        default=0.5,
                        type=float)
    ...
    # Store the position of the hand at the last brick drop point.
    ...
    while not found_box:
        ...
        box, image, vision_tform_box, objList = get_obj_and_img(
            network_compute_client, options.ml_service, options.person_model,
            options.confidence_person, kImageSources, 'box')
        
        # MOVING FOR THE BOX LIKE A ROOMBA
        ...
        walk_rt_vision, heading_rt_vision = compute_stand_location_and_yaw(
            vision_tform_box, robot_state_client, distance_margin=1.0)
        ...
        (center_px_x,
            center_px_y) = find_center_px(box.image_properties.coordinates)
        ...
        # # If we have already found the brick off, make sure it has moved a sufficient amount before
        # # picking it up again
        # if vision_tform_hand_at_drop is not None and pose_dist(
        #         vision_tform_hand_at_drop, vision_tform_brick) < 0.5:
        #     print('Found brick, but it hasn\'t moved.  Waiting...')
        #     time.sleep(1)
        #     continue
    ...
    # ASSUMED TO HAVE BEEN FOUND (RN PLACED IN FRONT OF)
    ...
    while True:
        # Capture an image and run ML on it.
        brick, image, vision_tform_brick, objList = get_obj_and_img(
            network_compute_client, options.ml_service, options.model,
            options.confidence_brick, kImageSources, 'brick')
        ...
        # # If we have already dropped the brick off, make sure it has moved a sufficient amount before
        # # picking it up again
        # if vision_tform_hand_at_drop is not None and pose_dist(
        #         vision_tform_hand_at_drop, vision_tform_brick) < 0.5:
        #     print('Found brick, but it hasn\'t moved.  Waiting...')
        ...
        runningBrickCount = len(objList)
        
        #Encoding image for cv2
        dtype = np.uint8
        img = np.fromstring(image.shot.image.data, dtype=dtype)
        if image.shot.image.format == image_pb2.Image.FORMAT_RAW:
            img = img.reshape(image.shot.image.rows,
        ...
        if (runningBrickCount > maxBrickCount):
            maxBrickCount = runningBrickCount
            fname = ("imgs" + str(maxBrickCount) + ".jpg")
            cv2.imwrite(fname, img)
        
        print('Found brick...', maxBrickCount)
...
#     while True:
#         holding_brick = False
#         while not holding_brick:
#             # Capture an image and run ML on it.
#             brick, image, vision_tform_brick = get_obj_and_img(
#                 network_compute_client, options.ml_service, options.model,
#                 options.confidence_brick, kImageSources, 'brick')
...
#             # Walk to the object.
#             walk_rt_vision, heading_rt_vision = compute_stand_location_and_yaw(
#                 vision_tform_brick, robot_state_client, distance_margin=1.0)
...
#             # The ML result is a bounding box.  Find the center.
#             (center_px_x,
#              center_px_y) = find_center_px(brick.image_properties.coordinates)
